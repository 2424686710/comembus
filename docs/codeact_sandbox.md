...
`run_code_sandbox()` 使用 `multiprocessing.Process` 执行代码，并通过预先创建的单向 `multiprocessing.Pipe` 回传结果。
...
- 超出限制时返回 `stdout_truncated=true`
- 用户代码必须定义 `result`
- worker 执行前设置 `RLIMIT_CPU`、`RLIMIT_AS`、`RLIMIT_FSIZE`、`RLIMIT_NOFILE`、`RLIMIT_NPROC`
- `RLIMIT_NPROC=0` 禁止 fork/子进程；Pipe 避免沙箱自身创建 Queue feeder thread
